...
router.post('/', [
  ...
  try {
    ...
    // Notify project participants (except comment author) - DON'T AWAIT, LET IT RUN IN BACKGROUND
    setImmediate(async () => {
      try {
        const notifyUsers = [
          project.projectManager,
        ...
        console.error('Failed to create notifications for comment:', notificationError.message);
      }
    });
