...
const { authenticate, authorize, isProjectManager } = require('../middleware/auth');
const { createNotification, createBulkNotifications } = require('../utils/notifications');
const { clientLogoUpload, deleteFile } = require('../utils/cloudinary');
...
router.post('/', 
  authorize('admin', 'project_manager'),
  clientLogoUpload.single('clientLogo'),
  [
    body('title').trim().notEmpty().withMessage('Project title is required'),
    body('description').optional().trim()
  ],
  async (req, res) => {
    try {
      ...
      if (clientId) {
        const client = await User.findById(clientId);
        if (!client || client.role !== 'client') {
          // Delete uploaded file from Cloudinary if client invalid
      ...
      // Add client logo if uploaded (Cloudinary URL)
      if (req.file) {
        projectData.clientLogo = {
          filename: req.file.filename,
          path: req.file.path, // Cloudinary URL (e.g., https://res.cloudinary.com/...)
      ...
      // Notify client if added (run in background)
      if (clientId) {
        setImmediate(async () => {
          try {
            await createNotification(
            ...
            console.error('Failed to send project notification:', notificationError.message);
          }
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        if (req.project.clientLogo && req.project.clientLogo.publicId) {
          try {
            await deleteFile(req.project.clientLogo.publicId);
            console.log('🗑️  Deleted old client logo from Cloudinary');
          } catch (deleteError) {
      ...
      // Notify team members (run in background)
      setImmediate(async () => {
        try {
          const notifyUsers = req.project.teamMembers.map(tm => {
            return tm._id ? tm._id : tm;
          ...
          console.error('Failed to send project update notifications:', notificationError.message);
        }
      });
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('Failed to send team member notification:', notificationError.message);
      }
    });
...
router.delete('/:id/team-members/:userId', isProjectManager, async (req, res) => {
  try {
    ...
    // Notify removed member (run in background)
    setImmediate(async () => {
      try {
        await createNotification(
          req.params.userId,
        ...
        console.error('Failed to send removal notification:', notificationError.message);
      }
    });
...
router.delete('/:id/client', isProjectManager, async (req, res) => {
  try {
    ...
    // Notify removed client (run in background)
    if (clientId) {
      setImmediate(async () => {
        try {
          await createNotification(
          ...
          console.error('Failed to send client removal notification:', notificationError.message);
        }
      });
    }
