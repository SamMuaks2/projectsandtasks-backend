...
const { body, validationResult } = require('express-validator');
const Settings = require('../models/Settings');
const { logoUpload, deleteFile } = require('../utils/cloudinary');
...
// Update company logo (admin only) - Now uses Cloudinary
router.post('/logo', authorize('admin'), logoUpload.single('logo'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: 'No logo file uploaded' });
    ...
    if (settings.companyLogo && settings.companyLogo.publicId) {
      try {
        await deleteFile(settings.companyLogo.publicId);
      } catch (deleteError) {
    ...
    // Update settings with new logo from Cloudinary
    settings.companyLogo = {
      filename: req.file.filename,
